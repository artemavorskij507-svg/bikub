@php
    ...
    if (isset($record) && $record) {
        ...
        $active = $activeServiceTypesCount;
        $inactive = $serviceTypesCount - $activeServiceTypesCount;
        $message = null;
    } else {
        $total = 0;
        $active = 0;
        $inactive = 0;
        $message = 'Статистика будет доступна после сохранения';
    }
@endphp

@if($message)
    <div class="p-4 rounded-lg bg-slate-50 border border-slate-200 text-center text-slate-600">
        {{ $message }}
    </div>
@else
    <div class="grid grid-cols-3 gap-4">
        ...
        <div class="p-3 rounded-lg bg-slate-50 border border-slate-100">
            <div class="text-2xl font-bold text-slate-600">{{ $inactive }}</div>
