@php
    // Данные передаются из Placeholder компонента
    $color = $color ?? '#3b82f6';
...
<div class="p-4 rounded-lg border border-slate-200 bg-gradient-to-br from-white to-slate-50 shadow-sm">
    <div class="flex items-center gap-3">
        <div class="w-14 h-14 rounded-xl flex items-center justify-center text-white text-xl font-bold shadow-lg transition-all duration-200 hover:scale-105" style="background: linear-gradient(135deg, {{ $color }} 0%, {{ $color }}dd 100%);">
            @if(isset($icon) && str_starts_with($icon, 'heroicon'))
                @php
                    $iconName = str_replace('heroicon-o-', '', $icon);
                @endphp
                <svg class="w-7 h-7" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    @if($iconName === 'truck')
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" />
                    @elseif($iconName === 'archive-box')
                        <path stroke-linecap="round" stroke-linejoin="round" d="M20.25 7.5l-.625 10.632a2.25 2.25 0 01-2.247 2.118H6.622a2.25 2.25 0 01-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125z" />
                    @elseif($iconName === 'wrench-screwdriver')
            ...
            @else
                <span class="text-2xl">{{ substr($name, 0, 1) }}</span>
            @endif
        </div>
        <div class="flex-1">
            <div class="font-bold text-slate-900 text-lg">{{ $name }}</div>
            ...
            <div class="mt-2 flex items-center gap-2">
                ...
                <span class="text-xs font-mono text-slate-600">{{ $color }}</span>
            </div>
        </div>
    </div>
</div>
