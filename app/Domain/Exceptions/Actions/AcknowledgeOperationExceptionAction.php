...
class AcknowledgeOperationExceptionAction
{
    public function __construct(
        private readonly WriteJobTimelineAction $writeJobTimelineAction,
    ) {}

    public function execute(OperationException $exception, int $userId): OperationException
    {
        $exception->update([
            'status' => 'acknowledged',
            'acknowledged_at' => now(),
        ...
        if ($job) {
            $this->writeJobTimelineAction->execute(
                job: $job,
                eventType: 'operation_exception_acknowledged',
