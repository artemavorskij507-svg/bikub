...
use App\Events\Operations\SlaBreached as OpsSlaBreached;
use App\Models\Operations\ServiceJob;
use App\Models\Operations\SlaTimer;
use Illuminate\Support\Facades\Log;

class BroadcastSlaBreachedBridge
{
    public function handle(object $event): void
    {
        ...
        try {
            event(new OpsSlaBreached($job, $phase, $timer));
        } catch (\Throwable $e) {
            Log::warning('BroadcastSlaBreachedBridge failed', [
                'job_id' => $job->id,
                'error' => $e->getMessage(),
            ]);
        }
